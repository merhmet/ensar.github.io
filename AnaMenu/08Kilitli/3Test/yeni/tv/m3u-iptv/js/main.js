...
aFavourites = false, aPlaylistHistory = false, aChannelList = [], aFilteredChannelList = [], oSelectedItem = false,
...
function downloadPlaylistAjax( sUrl, sCallback ) {
	...
	if( sUrl === "https://raw.githubusercontent.com/Free-TV/IPTV/master/playlist.m3u8" && typeof(sDemoPlaylist) !== 'undefined' ) {
...
function getCurrentSelectedItem() {
	...
	if( bHistoryBrowserOpened ) {
		...
		var oSelected = getFocusedHistoryItem();

	} else if( bChannelSettingsOpened ) {
		var oSelected = document.querySelector('#channel_settings_nav li.selected');
		...
		var oSelected = document.querySelector('#group_list li.selected');
...
function selectListItem() {
	...
	if( bGroupsOpened ) {
		...
		if( sSelectedGroup === '-' ) {
			sSelectedGroup = false; // All channels
		}
...
function getNextChannel() {
	var oSelected = getCurrentSelectedItem();
	var oSelectedItem = oSelected.nextElementSibling;
	if( !oSelectedItem ) {
		oSelectedItem = oSelected.parentElement.firstElementChild;
	}
	return oSelectedItem;
...
function getPrevChannel() {
	var oSelected = getCurrentSelectedItem();
	var oSelectedItem = oSelected.previousElementSibling;
	if( !oSelectedItem ) {
		oSelectedItem = oSelected.parentElement.lastElementChild;
	}
	return oSelectedItem;
...
function buildNav( bSkipCurrentChannelSelect ) {
	...
	if( sSelectedGroup === '__fav' && iFavChannels === 0 ) {
		showModal(getLang("errorNoFavouritesYet"));
		oSelectedItem = getEl('favourites_group');
		oSelectedItem.classList.add('selected');
		showGroups();
		return false;
	}
