...
function simp_setAlbum(index) {
  ...
  simp_title.innerHTML = simp_source[index].querySelector(
    ".simp-source"
  ).innerHTML;
...
if (document.querySelector("#simp")) {
  ...
  simp_elem +=
    '<div class="simp-others flex flex-align"></button><button type="button" class="simp-random fa fa-random" title="Random"></button><div class="simp-shide"><button type="button" class="simp-shide-top fa fa-caret-up" title="Show/Hide Album"></button><button type="button" class="simp-shide-bottom fa fa-caret-down" title="Show/Hide Playlist"></button></div></div>';
...
/* взято отсюда https://codepen.io/sekedus/pen/ExxjZEz*/